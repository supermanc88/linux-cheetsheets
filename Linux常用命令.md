...
linux链接分两种，一种被称为硬链接(hard link)，一种被称为称号链接(symbolic link)。默认情况下，ln命令产生硬链接。
...
```bash
docker run -p 0.0.0.0:36022->22 -it ubuntu /bin/bash
...
如果不给`skill`变量加花括号，写成`echo "I am good at $skillScript"`，解释器就会把`$skillScript`当成一个变量(其值为空)，代码执行结果就不是期望样子。
...
**这样写是合法的，但需要注意的是，第二次赋值的时候不能写`$your_name="alibaba"`，只有在使用变量时候才加美元符`$`。**
...
- 变量只能在函数内部使用，叫做局部变量
- 变量在当前shell中使用，叫做全局变量
- 变量还可以在子进程中使用，叫做环境变量

shell函数中定义的变量默认也是全局变量，它在函数外部定义变量拥有一样的效果：
...
如果在定义变量时加上`local`命令，此时该变量就成也局部变量
...
虽然上述两种方式都可以在 Shell 脚本中得到命令运行的结果，但是有一点是不一样的，那就是**反引号执行命令不支持嵌套**，不能实现反引号中再出现反引号，而 `$(command)`的方式是支持嵌套的，关于这一点可以看下面这个例子。
...
分析一下这个命令 `echo $(ls $(pwd))`，最里面的命令是 `$(pwd)`先执行得到当前目录，然后执行命令 `$(ls 当前目录)`得到目录下的文件，再通过 `echo` 命令把这个结果输出，就得到了`file1.txt` 这个文件名，因为我这个目录下只有这一个文件。
...
我们还是仿照上面嵌套来写，但是 `echo` 后面的内容其实被分成了3部分，一个`ls`命令，一个`pwd`字符串、一个空命令，这样就能解释运行结果 `file1.txtpwd`了。
...
**`$?`在函数调用完成后马上使用，中间插入几条命令之后，`$?`的值可能就不是函数的返回值了。**
...
```bash
chengheming@chengheingdeMBP Source % sort test.txt
...
2. uniq只能对相邻的数据去重，所以需要先排序

```bash
chengheming@chengheingdeMBP Source % sort test.txt | uniq
...
```bash
chengheming@chengheingdeMBP Source % sort -u test.txt
